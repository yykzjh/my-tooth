...
clip_lower_bound = 0.1  # clip的下边界百分位点，图像中每个体素的数值按照从大到小排序，其中小于30%分位点的数值都等于30%分位数
...
elastic_transform_alpha = 1.1  # 形变的幅度，值越大，弹性形变越剧烈

# 高斯噪声参数
gaussian_noise_mean = 0  # 高斯噪声分布的均值
gaussian_noise_std = 0.02  # 高斯噪声分布的标准差,值越大噪声越强
...
single_image_path = r"./datasets/src_10/val/images/19_2.nrrd"  # 单张图像的存储路径

single_label_path = "./datasets/src_10/val/labels/19_2.nrrd"  # 单张图像对应的标签存储路径
